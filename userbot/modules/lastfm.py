...
from userbot.modules.dbhelper import (getartist, getlastfmcheck, getLogging,
                                      getsong, getuserID, setartist,
                                      setlastfmcheck, setLogging, setsong,
                                      setuserID)
...
async def get_curr_track(lfmbio):
    ...
    while getlastfmcheck():
        try:
            ...
            if playing is not None and await getsong() != oldsong and await getartist() != oldartist:
                environ["oldsong"] = await getsong()
                environ["oldartist"] = await getartist()
                if BIOPREFIX:
                    lfmbio = f"{BIOPREFIX} 🎧: {await getartist()} - {await getsong()}"
                else:
                    lfmbio = f"🎧: {await getartist()} - {await getsong()}"
                try:
                    if BOTLOG and await getLogging():
                        await bot.send_message(
                            BOTLOG_CHATID,
                            f"Attempted to change bio to\n{lfmbio}")
                    await bot(UpdateProfileRequest(about=lfmbio))
                except AboutTooLongError:
                    short_bio = f"🎧: {await getsong()}"
